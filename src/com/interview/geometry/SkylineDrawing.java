...
public class SkylineDrawing {
    ...
    static class BuildingPoint implements Comparable<BuildingPoint> {
        ...
        public int compareTo(BuildingPoint o) {
            ...
            //if one start and one end is compared then start should be picked before end
            if (this.x != o.x) {
                return this.x - o.x; // smaller x comes first
            } else {
                // isStart is given negative value to prioritize
    ...
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        //Data structure used must support: add, remove, returnMax
        ...
        /* key: height, value: count of height */
        ...
        for(BuildingPoint buildingPoint : buildingPoints) {
            ...
            //peek the current max height after addition or removal of building x.
            int currentMaxHeight = queue.lastKey(); //return highest key in map
            //int currentMaxHeight = queue1.peek();
            //if height changes from previous height then this building x becomes critcal x.
            // So add it to the result.
            if (prevMaxHeight != currentMaxHeight) { // max height changed
                // add current buiding's x, current max height
                result.add(new int[]{buildingPoint.x, currentMaxHeight});
                prevMaxHeight = currentMaxHeight; // track max height
            }
        }
        return result;
    }
